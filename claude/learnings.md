...
### Secret-listing endpoints return metadata, never the secret; reveal-once at creation

> The dashboard needs to show a bot's API tokens so the owner can revoke them. Should the list endpoint return the tokens?

No - it returns *metadata about* the tokens, never the tokens themselves. The `bot_tokens` table stores only a `tokenHash` (a one-way hash of `pbt_<hex>`), so the raw token is unrecoverable by design; the list endpoint (`GET /tokens`) selects id/name/createdAt/lastSeenAt/revokedAt and nothing that could reconstruct the credential. The raw value exists exactly once, in the mint response (`POST /tokens` → `{ rawToken }`), and the UI reveals it a single time with a copy button and "shown once" copy. This is the same shape as GitHub PATs, AWS secret keys, and this project's own `email_verification_tokens` / `password_reset_tokens`: **store a hash, show the plaintext once, list only metadata.** The lesson: when building a "manage my credentials" screen, the read path and the create path have different security contracts - the create path may briefly surface the secret, the read path never may. If your list endpoint can return the secret, the storage model is already wrong (you're keeping plaintext or a reversible form).

The rotation corollary: "rotate" is not one atomic operation for a credential that lives in someone else's running config. Auto-revoking the old token the instant you mint a new one guarantees an outage between mint and redeploy. The safe rotate is two independent steps - generate-new, then revoke-old after the new one is in place - which also falls out naturally from modeling tokens as an append-only list with per-row `revokedAt` rather than a single mutable "current token" field.

### A server-rendered layout won't reflect a client mutation until you revalidate/refresh

> After publishing a bot with a client-side fetch, why did the sidebar (rendered in the server layout) still show nothing until a manual reload?

In the Next.js App Router, a layout is a React Server Component: it runs once on the server, queries the DB, and streams HTML/RSC to the client. A client component nested inside it (the bot factory form) can `fetch()` and mutate the database, but that does **not** re-run the server layout - the client has no reason to re-request the RSC payload, so the sidebar keeps its original (empty) render. Two mechanisms fix this: `router.refresh()` (client-side - forces a re-fetch of the current route's RSC and re-renders server components while **preserving** client-component state, so the user stays on step 5), and `revalidatePath("/")` inside a server action (server-side - marks that path's cache stale so the next render is fresh). Using both belt-and-suspenders is fine. The key mental model: **client `fetch` mutations are invisible to already-rendered server components; you must explicitly invalidate.** This is the RSC analogue of "the DOM won't update itself when the database changes" - except here the "DOM" is server-computed and the trigger is `refresh()`/`revalidatePath`, not `setState`.

A second, subtle point: `router.refresh()` re-runs the server page that supplies a client component's props, but it does **not** reset that client component's `useState`. `useState` initializers only run on mount; new props flowing in from a refreshed parent are reconciled without clobbering internal state. That's why refreshing after publish updates the sidebar yet leaves the wizard sitting on step 5 with `published = true` - exactly the desired "update the chrome, keep my place" behavior. If you actually want fresh props to reset a client subtree, you have to change its `key`.

### "Scheduled" is not "running": a cron-gated job that hard-fails on a missing secret is a silent no-op

> A daily cron was configured to purge deleted accounts, yet an account sat 24 days past its purge date, unpurged, and could still log in. Why?

Three independent links have to all hold for a scheduled cleanup to actually happen, and the failure was in the middle one: **scheduler → authorization → job logic.** `vercel.json` scheduled the cron (link 1 ✓), and `runPurgeJob` was correct (link 3 ✓) - but the route guards itself with `if (!process.env.CRON_SECRET) return 503`, and `CRON_SECRET` was never set in the deployment (link 2 ✗). So Vercel dutifully hit the endpoint every day and every call bounced with 503. Nothing logged an error the operator would notice; the job just silently never ran. The lesson: when a background/cleanup job "isn't happening," don't start by debugging the job's logic - **trace the whole chain and confirm the job is even being invoked and authorized.** A route that returns early on a missing secret is invisible-by-design; pair it with an alert (or at least treat a persistent 503 on a cron path as a page-worthy signal), because a security guard that silently disables the job is worse than a job that loudly crashes.

Corollary on soft-delete UX: this system deletes accounts via a 7-day-grace `deletion_requests` row and a purge job, which is the right pattern (undo window, no immediate data loss). But it means the account genuinely still exists during grace - and if the purge never runs, "deleted" accounts stay fully functional forever. Any soft-delete-with-grace design has an implicit hard dependency on the reaper actually running; the reaper is not optional polish, it's what makes "deleted" eventually true. Guard the UI for the overdue state too (past the purge date but not yet purged) rather than assuming the countdown is always positive.
