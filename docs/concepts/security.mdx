...
A 429 response includes the `scope` (which tier tripped) and `resetAt` (epoch ms). The limiter is in-process - fine for Vercel single-region; for multi-region production you would switch to Redis (Upstash) per Stage 7 of the [roadmap](/concepts/stages).
...
See [Roadmap & stages](/concepts/stages).
