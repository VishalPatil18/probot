...
The chat route accepts an optional `x-llm-api-key` header (used by the owner's own dashboard test chat and by self-hosted deployments) and falls back to the managed encrypted key otherwise:

1. **Header present** (owner testing from their own browser, self-hosted runtime) - used directly.
...
- **Azure keys are header-only.** Azure's multi-secret credential (key + endpoint + apiVersion) isn't supported by managed storage in this release, so Azure bots require the owner to be online with the credentials in their browser store, or self-hosting via `probot-bot`. Other providers all go through managed encryption.
- **Self-hosting the runtime.** If you'd rather not trust any operator, run the `probot-bot` runtime on your own infrastructure - the plaintext key lives entirely inside your deployment. See [Self-hosted bot](/self-hosted-bot/index).
...
- **Full infrastructure compromise.** Anyone with deploy access can read the KEK. If that's your threat model, self-host the `probot-bot` runtime.
