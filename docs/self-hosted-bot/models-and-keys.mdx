...
<CardGroup cols={2}>
  ...
  <Card title="OpenAI-compatible" icon="plug">
    `createOpenAIHandler` with `baseUrl` — Grok, Ollama, Azure OpenAI, LM
...
| Ollama | No key — runs locally | Whatever you pulled (e.g. `llama3.2`) | Free, local, no key. |
...
<Tip>
  Gemini's free tier and local Ollama are the two genuinely $0 paths.
  Great for prototyping or side projects.
</Tip>
...
### OpenAI-compatible endpoints (Grok, Ollama, Azure, …)
...
// Ollama (no key)
const sendOllama = createOpenAIHandler({
