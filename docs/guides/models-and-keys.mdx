...
Ollama needs no key - just the **base URL** of your Ollama server (default `http://localhost:11434`) and the **model name** you've pulled (e.g. `llama3.2`). The base URL is stored in your browser and sent per request in the `x-llm-ollama-base-url` header. Plain `http://` is allowed only for localhost; any remote Ollama must be served over `https://`.
